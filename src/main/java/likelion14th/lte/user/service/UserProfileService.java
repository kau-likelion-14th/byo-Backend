...
public class UserProfileService{
    ...
    // 답변: 직접 객체를 생성하면 클래스 간 결합도가 높아져 유지보수가 어려워짐. DI를 통해 외부에서 의존성을 주입받으면 결합도를 낮출 수 있고, 테스트 시 Mock 객체를 주입하기 쉬워져 단위 테스트의 효율성 향상됨.
    ...
    */
    ...
    public UserProfileResponse createTestUser(CreateTestUserRequest request){
        ...
        // 답변: 어떤 필드에 어떤 값을 넣는지 파악이 가능해 가독성이 좋아지며, 생성자의 인자 순서와 상관없이 안전하게 객체 생성 가능. 또한 선택적 인자가 많은 경우에도 유연하게 대처
        ...
        try{
            ...
            // 답변: 트랜잭션은 작업의 원자성을 보장. 저장 도중 DB 서버가 끊기는 등의 예외가 발생하면, 해당 메서드 내에서 이루어진 모든 작업을 취소해 데이터의 일관성을 유지하기 때문에 필요함.
